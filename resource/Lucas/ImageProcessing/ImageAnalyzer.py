...
class ImageAnalyzer:
    ...
    def compute_mean_visibility(self):
        ...
        self.save_image("images/output/")
        print(f"Mean visibility: {self.mean_visibility:.2f}")
    ...
    def save_image(self, file_name: str):
        ...
        self.edge_localisation = 100 * self.edge_localisation
        cv.imwrite(file_name + "_filtered_derived.png", self.derived_filtered_img)
        cv.imwrite(file_name + "_edges.png", self.edge_localisation)
        cv.imwrite(file_name + "_visibility_map.png", self.visibility_map)
        print("save",file_name)

    def save_visibility_map(self, file_name: str):
        # visibility_normalized_array = cv.normalize(self.visibility_map, None, 0, 255, cv.NORM_MINMAX, cv.CV_8U)
        # cv.imwrite(file_name + ".png", visibility_normalized_array)
        cv.imwrite(file_name + ".jpg", self.visibility_map)
